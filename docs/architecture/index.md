# Architecture

## Current Shape

- Kind: runnable macOS desktop shell; capture behavior is not implemented yet.
- Stack: Tauri v2 desktop shell with a Svelte 5/TypeScript/Vite interface,
  framework-neutral Motion animations, and a Rust capture and retention core.
  The MVP uses ScreenCaptureKit and VideoToolbox on macOS; FFmpeg is a bundled
  local muxing sidecar. Platform adapters preserve a path to later Windows
  support.

## Implemented System Map

- `src/main.ts` mounts the Svelte application and global style layers.
- `src/App.svelte` owns local preview state for capture target and replay-window
  selection. It invokes no native commands and labels capture/save behavior as
  unavailable.
- `src/window.css` owns global tokens and desktop window chrome;
  `src/app.css` owns the replay workspace presentation.
- `src-tauri/src/lib.rs` is the native entry boundary. It currently starts the
  main Tauri window without plugins, commands, permissions, or background work.
- `src-tauri/tauri.conf.json` connects Vite's development/build output to the
  native shell and defines the `com.gisketch.encore` application identity.

## Planned Runtime Responsibilities

- Capture selects a full display or application window and emits timestamped
  video frames through the platform capture API.
- Encoder converts frames to H.264 with the platform hardware encoder and a
  one-second keyframe interval.
- Segment retention persists 10-second fragmented-MP4 chunks and deletes chunks
  outside the selected replay window.
- Trigger receives a global hotkey while Encore is unfocused.
- Packager concatenates retained chunks without re-encoding and writes a local
  MP4 with adjacent metadata and available logs.
- Desktop UI owns capture selection, retention settings, permission guidance,
  status, and access to saved clips. Animation is presentation-only and must
  respect the operating system's reduced-motion preference.
- Platform adapters own Windows/macOS capture, hardware encoder, and permission
  differences; the retention and packaging policy remains platform-neutral.

These are responsibility boundaries, not implemented modules. Add them one
public seam at a time; the interface must not present a planned capability as
active before its native implementation exists.

## Current Boundaries

| Boundary | Owns | Allowed dependencies | Validation |
|---|---|---|---|
| Desktop UI | Preview state and presentation | Svelte, Motion, browser APIs | `npm run check && npm run build` |
| Native shell | Window lifecycle and future native composition | Tauri core | Rust format, Clippy, and tests from `src-tauri` |
| UI/native seam | Future typed commands and events | Nothing yet | Add a public-seam test with the first command |

## Boundary Rule

For each load-bearing boundary, record:

- What it owns.
- Its public interface.
- Allowed dependencies.
- Relevant validation command.

If a boundary must stay true, enforce it mechanically.
