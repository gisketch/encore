...
/// `scan`) to its replay file inside `destination`. `id` always comes from
/// the frontend, so this is the one seam in the library that has to
...
pub(crate) fn resolve_replay_file(destination: &Path, id: &str) -> Result<PathBuf, String> {
    ...
    Ok(bundle.join(crate::packager::REPLAY_FILENAME))
}
